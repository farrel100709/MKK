...
// --- Ambil filter tahun ---
$tahun_filter = isset($_GET['tahun']) ? $_GET['tahun'] : $tahun_sekarang;

// --- Ambil data tamu terakhir ---
$tamu_terakhir = mysqli_query($koneksi, "SELECT * FROM tamu ORDER BY tanggal DESC LIMIT 1");
$tamu = mysqli_fetch_assoc($tamu_terakhir);

// --- Ambil data grafik perbulan berdasarkan tahun filter ---
...
<div class="container">
            ...
            <div class="input-group">
                ...
                <select name="tahun" id="tahun" class="form-select" onchange="this.form.submit()">
                    <?php for ($t = 2019; $t <= $tahun_sekarang; $t++): ?>
                        <option value="<?= $t ?>" <?= ($t == $tahun_filter) ? 'selected' : '' ?>><?= $t ?></option>
                    <?php endfor; ?>
                </select>
            </div>
        </form>
    ...
    <div class="text-center">
        ...
        <button class="btn btn-success btn-custom" onclick="downloadChart()">⬇️ Download Grafik</button>
    </div>
</div>
